...
采集平台可任选子集：首页勾掉的平台**不会被启动**（请求带 `platforms[]`），
多选时按来源轮转均衡采样。

另有一层**检索增强**，与上面的评论采集并发执行，同样可单独勾选：

| 检索源 | 匿名可用 | 备注 |
|---|:---:|---|
| 百度搜索 | ✅ | 覆盖面广；频繁请求会触发安全验证，稍等即恢复 |
| 必应搜索 | ✅ | 风控宽松，百度被限流时的兜底 |

两个引擎并发跑，结果按来源轮转合并，并跨引擎按 URL 与标题去重。
...
> 部分选项后端尚未支持（排序策略、情感粒度、GitHub 检索源等），
> 界面上会标注 **前端预设**，只保存偏好、不谎称已生效。

### 采集

`auto` 模式多源并发，单源首批 8 s 未返回自动跳过、总时长上限 60 s；
所有源都有定论后立即开闸，不再干等满 8 s。
各源结果**按来源轮转合并**并跨源去重——直接按到达顺序拼接的话，最快的源会吃光整个配额，
"多源聚合"很容易变成九成来自同一个平台，情感分布也就跟着失真。
结果页的「样本构成」卡片会显示各来源的实际占比。进度经 WebSocket 实时推送。

### 清洗

除了剥标签、去 URL/@/话题/表情，还会做**近似去重**（抹掉标点后比对，
"太好了！！！"与"太好了。"算同一条）和**引流评论过滤**（要求"动作 + 渠道"
同现，"微信支付很方便"这类正常讨论不受影响）。
...
| 模块 | 覆盖 |
|---|---|
| `test_search.py` | 百度 / 必应解析（广告过滤 / 跳转还原 / 占位地址 / rank 连续性）、文本归一化、统一模型校验、provider 注册与重名冲突、聚合层的失败隔离、超时隔离与跨引擎去重 |
| `test_preprocess.py` | 标记与 base64 剥离、近似去重、引流评论过滤（含正常讨论不误伤） |
| `test_auto.py` | 聚合爬虫的来源均衡、跨源去重、慢源超时、平台子集、配额满后取消 |
