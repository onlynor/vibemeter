...
class WeiboCrawler(BaseCrawler):
    ...
    async def ping(self) -> PingResult:
        """探一次搜索接口，顺带识别访客系统拦截与 Cookie 失效"""
        if not self._cookies.get("SUB"):
            return False, "未配置 WEIBO_COOKIE（或缺少 SUB 字段），微博搜索强制登录态"
        async with make_client(referer=f"{self.BASE}/") as client:
            for k, v in self._cookies.items():
                client.cookies.set(k, v, domain=".weibo.cn")
            await self._warm_up(client)
            try:
                posts = await self._search_posts(client, "电影", limit=1)
            except RuntimeError as exc:
                return False, str(exc)
        if not posts:
            return False, "微博搜索无返回，Cookie 可能已过期或正在风控"
        return True, "微博搜索可用（Cookie 有效）"
