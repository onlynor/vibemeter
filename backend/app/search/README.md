...
> 前端也按这个定位摆放：结果页先出「平台内容来源」（评论真正的出处），
> 检索结果排在分析结果之后、默认收起的「事件背景」卡里。它是背景资料，
> 不该是用户搜完关键词第一眼看到的东西。
...
时限与条数全部集中在 `app/config.py`，并支持环境变量覆盖，换网络环境时不必改代码：

| 变量 | 默认 | 含义 |
|---|---|---|
| `VIBE_SEARCH_TIMEOUT` | 12s | 单个 provider 的检索时限，超时只淘汰它自己 |
| `VIBE_SEARCH_WARMUP_TIMEOUT` | 6s | 百度预热的独立时限，必须远小于上一项 |
| `VIBE_SEARCH_COOKIE_TTL` | 900s | 预热 Cookie 的复用时长 |
| `VIBE_SEARCH_LIMIT` | 8 | 单引擎取数上限 |
| `VIBE_SEARCH_TOTAL_LIMIT` | 12 | 合并去重后的总量上限 |
| `VIBE_SEARCH_TITLE_DEDUP_MIN_LEN` | 8 | 参与标题去重的最短归一化标题 |
| `VIBE_SEARCH_OVERFETCH` | 10 | 向引擎多要几条，抵掉广告与非结果容器 |

## 百度实现要点

- 必须先访问一次 `www.baidu.com` 拿 `BAIDUID` 再检索。直连 `/s` 只能拿到
  7 个左右的容器且多为卡片；带 Cookie 后是完整一页（实测 20 容器 / 19 自然结果）。
- 这次预热是**串行**的一次往返（实测约 270ms），所以 Cookie 在进程内按 TTL
  缓存复用，只有冷启动与过期后才真的去打首页；并发冷启动由一把锁收敛成一次请求。
  预热另有独立的短时限：它只是为了拿一个 Cookie，卡在这里等于白耗掉整次检索的预算，
  失败也不致命——照常裸检索。
